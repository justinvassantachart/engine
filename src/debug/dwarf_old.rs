...
pub fn parse_debug_info(wasm_bytes: &[u8]) -> anyhow::Result<DebugInfo> {
    ...
    while let Some(header) = units.next()? {
        let unit = gimli_dwarf.unit(header)?;
        parse_unit_functions(&gimli_dwarf, &unit, &mut functions)?;
        parse_unit_lines(&gimli_dwarf, &unit, &mut locations, &mut files, &mut file_map)?;
    }
